...
if page == "Home":
    ...
    st.write("Welcome to the Traffic Data Dashboard!")
...
# Exploration page
    ...
    st.title("State/UT/City Wise Overtaking Cases")
    # Pivot the data
    occupation_plot = df.pivot_table(index='State/UT/City', values='Overtaking Cases', aggfunc=np.mean)
    # Display the bar chart using Streamlit
    st.bar_chart(occupation_plot)

    st.title("State/UT/City Wise Overtaking Injured")
    # Pivot the data
    occupation_plot = df.pivot_table(index='State/UT/City', values='Overtaking Injured', aggfunc=np.mean)
    # Display the bar chart using Streamlit
    st.bar_chart(occupation_plot)
...
# Linear Regression page
    ...
    selected_state = st.selectbox("Select State", df["State/UT/City"].unique())
    ...
    # Add the selected state to the user input
    features["State/UT/City"] = selected_state
    ...
    # Prediction function
    def predict():
        prediction = lr.predict(user_input.drop("State/UT/City", axis=1))
        return prediction[0]
